...
Educational monitoring agent without GUI dependencies
"""
import os
import sys
import json
...
class SimpleAgent:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print("[OK] Configuration created with default values")
        except Exception as e:
    ...
    def get_consent(self) -> bool:
        ...
        print("\n" + "="*60)
        print("EDUMON - EDUCATIONAL MONITORING CONSENT")
        print("="*60)
        print("\nThis agent will send ONLY the following data:")
        print("[OK] Anonymous device identifier")
        print("[OK] Host name and system username")
        print("[OK] Performance metrics: CPU, RAM, disk, network")
        print("[OK] System uptime")
        print("[OK] Process information (names only)")
        print("\n[NO] NEVER captured:")
        ...
        print("\n[STOP] You can stop monitoring anytime with Ctrl+C")
        print("="*60)
        
        while True:
            response = input("\nDo you agree to participate in this monitoring session? (yes/no): ").strip().lower()
    ...
    def register_with_server(self) -> bool:
        ...
        try:
            ...
            print(f"[CONNECT] Connecting to server: {self.config['server_url']}")
            ...
            if response.status_code == 200:
                ...
                print(f"[OK] Registration successful. Session ID: {self.session_id}")
                return True
            else:
    ...
    def send_heartbeat(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[WARNING] Error sending metrics: {response.status_code}")
                return False
    ...
    def unregister_from_server(self, reason: str = "user_request") -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                print("[OK] Unregistration successful")
                return True
            else:
                print(f"[WARNING] Unregistration error: {response.status_code}")
                return False
                
        except Exception as e:
            print(f"[ERROR] Unregistration error: {e}")
            return False
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if not self.get_consent():
                print("[STOP] Consent not granted")
                return 0
            ...
            print("\n[START] AGENT STARTED")
            print("=" * 40)
            print(f"[ID] Device ID: {self.device_id}")
            print(f"[SERVER] Server: {self.config['server_url']}")
            print(f"[CLASSROOM] Classroom: {self.config.get('classroom_id', 'Not configured')}")
            print(f"[INTERVAL] Interval: {self.config.get('heartbeat_seconds', 15)} seconds")
            ...
            while self.running:
                try:
                    ...
                    if not self.send_heartbeat():
                        print("[WARNING] Heartbeat failed, continuing...")
            ...
            print("\n[STOP] Agent stopped correctly")
            return 0
            
        except Exception as e:
            print(f"[ERROR] Error running agent: {e}")
            return 1
        finally:
            self.running = False
