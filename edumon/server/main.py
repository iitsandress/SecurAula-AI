...
import time
from . import retention as retention_module  # type: ignore

from fastapi import FastAPI, Request, HTTPException, Header, Depends
from pydantic import BaseModel, Field
from . import dashboard as dashboard_module  # type: ignore
from . import control as control_module  # type: ignore
...
# Montar dashboard y control protegidos por API Key
try:
    from . import dashboard, control
    app.include_router(dashboard.router)
    app.include_router(control.router)
except Exception:
    # modo relativo cuando se ejecuta como módulo simple
    import importlib
    try:
...
async def list_clients(_: str = Depends(require_api_key)) -> Dict[str, Any]:
    # Política de retención ligera por lectura
    try:
        import importlib
        try:
        ...
        retention.purge_old_clients(time.time())
        retention.rotate_audit_log(time.time())
    except Exception:
        pass
    clients = load_clients()
    # Devolver información acotada y transparente
